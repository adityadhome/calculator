...
gui.title("Sientific Calculator ©Aditya Dhome") 
...
def exit():
    result = messagebox.askyesno("Exit","Would you like to exit the application?")
    if result:
        gui.destroy()
    else:
        return
...
titleframe = Label(gui,width=16,height=2,pady=5,font=('B titr',11),fg='white',bg='#35363A',text='©Aditya Dhome')
titleframe.place(x=472,y=15)

exitframe = Button(gui,width=15,height=1,font=('B titr',10),fg='white',bg='#840808',text='Exit program',command=exit)
exitframe.place(x=482,y=100)

btnhistory = Button(gui,bg='#F0A440',width=17,height=1
                    ,relief='raised',borderwidth=3,font=('B titr',9),fg='black',text='Clear history',command=history_clear)
